...
namespace SignaloBot.DAL
{
    public class CoreTVP
    {
        ...
        public static SqlParameter ToUserReceivePeriodType(string paramName, List<UserReceivePeriod> items, string prefix)
        {
            DataTable dataTable = items.ToDataTable(new List<string>()
            {
                ReflectionExtensions.GetPropertyName((UserReceivePeriod t) => t.PeriodOrder),
                ReflectionExtensions.GetPropertyName((UserReceivePeriod t) => t.PeriodBegin),
                ReflectionExtensions.GetPropertyName((UserReceivePeriod t) => t.PeriodEnd)
            });
        ...
        public static SqlParameter ToUpdateUserType(string paramName, List<Signal> items, string prefix)
        {
            ...
            DataTable dataTable = groupedMessages.ToDataTable(new List<string>()
            {
                ...
                ReflectionExtensions.GetPropertyName((Signal t) => t.DeliveryType),
                ReflectionExtensions.GetPropertyName((Signal t) => t.CategoryID),
                ReflectionExtensions.GetPropertyName((Signal t) => t.TopicID)
            });

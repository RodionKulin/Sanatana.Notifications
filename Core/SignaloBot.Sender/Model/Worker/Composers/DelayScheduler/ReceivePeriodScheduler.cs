...
{
    public class ReceivePeriodScheduler<TKey> : IDelayScheduler<TKey>
        where TKey : struct
    {     

        //методы
        public virtual DateTime GetSendTime(string timezoneID, List<UserReceivePeriod<TKey>> periods, out bool isDelayed)
        {
            ...
            UserReceivePeriod<TKey> nowPeriod = periods.FirstOrDefault(
        ...
        protected virtual DateTime FindClosestSendDate(List<UserReceivePeriod<TKey>> periods, DateTime nowTime)
        {
            List<UserReceivePeriod<TKey>> todayPeriods = periods.Where(p => p.PeriodBegin >= nowTime.TimeOfDay).ToList();
            List<UserReceivePeriod<TKey>> tomorrowPeriods = periods.Where(p => p.PeriodBegin <= nowTime.TimeOfDay).ToList();
            DateTime sendDate;
